...
const AdminSubjects = () => {
  ...
  const fetchSubjects = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/subjects/${selectedClass}`);
  ...
  const handleSubmit = async (values) => {
    setLoading(true);
    try {
      await axios.post('http://localhost:5000/api/subjects', values);
  ...
  const handleDelete = async (subjectCode) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`http://localhost:5000/api/subjects/${subjectCode}`);
