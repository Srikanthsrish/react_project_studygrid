...
const Subjects = () => {
  ...
  const [predefinedSubjects, setPredefinedSubjects] = useState({
    "LKG": ["Math", "Science", "English"],
    "UKG": ["Math", "Science", "English"],
    "1": ["Math", "Science", "English", "History", "Geography"],
    "2": ["Math", "Science", "English", "History", "Geography"],
    "3": ["Math", "Science", "English", "History", "Geography"],
    "4": ["Math", "Science", "English", "History", "Geography"],
    "5": ["Math", "Science", "English", "History", "Geography"]
  });
  ...
  useEffect(() => {
    if (selectedClass) {
      fetchSubjects(selectedClass); // Fetch subjects when class is selected
    }
  }, [selectedClass]);
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <select onChange={(e) => setSelectedClass(e.target.value)} value={selectedClass}>
          <option value="">--Select--</option>
          {["LKG", "UKG", "1", "2", "3", "4", "5"].map((classItem) => (
      ...
      {selectedClass && (
        <>
          ...
          <table border="1" style={{ width: "100%", textAlign: "left", marginTop: "20px" }}>
            ...
            <tbody>
              {subjects.length === 0
                ? predefinedSubjects[selectedClass].map((subject, index) => (
                    <tr key={index}>
                      <td>{subject}</td>
                      <td>
                        <button onClick={() => alert(`Viewing Subject: ${subject}`)}>View</button>
                        <button onClick={() => alert(`Editing Subject: ${subject}`)}>Edit</button>
                        <button onClick={() => alert(`Deleting Subject: ${subject}`)}>Delete</button>
                      </td>
                    </tr>
                  ))
                : subjects.map((subject) => (
